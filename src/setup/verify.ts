...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    ...
    // Ensure XDG_RUNTIME_DIR is set — required for systemctl --user in su/container sessions
    const xdgEnv = {
      ...process.env,
      XDG_RUNTIME_DIR: process.env.XDG_RUNTIME_DIR ?? `/run/user/${process.getuid?.() ?? 1000}`,
    };
    try {
      execSync(`${prefix} is-active nanoclaw`, { stdio: 'ignore', env: xdgEnv });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8', env: xdgEnv });
