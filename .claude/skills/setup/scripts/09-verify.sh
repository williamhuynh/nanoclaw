...
if [ -f "$PROJECT_ROOT/store/messages.db" ]; then
  if command -v sqlite3 >/dev/null 2>&1; then
    ...
    REGISTERED_GROUPS=$(node -e "const D=require('better-sqlite3');const d=new D('$PROJECT_ROOT/store/messages.db');console.log(d.prepare('SELECT COUNT(*) as c FROM registered_groups').get().c);d.close();" 2>/dev/null || echo "0")
  fi
fi
